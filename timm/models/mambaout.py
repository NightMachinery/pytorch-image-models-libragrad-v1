...
class MlpHead(nn.Module):
    ...
    def __init__(
        ...
        if hidden_size:
            self.pre_logits = nn.Sequential(OrderedDict([
                ('fc', NightLinear(in_features, hidden_size)),
                ('act', act_layer()),
        ...
        self.fc = NightLinear(self.num_features, num_classes, bias=bias)
    ...
    def reset(self, num_classes: int, pool_type: Optional[str] = None, reset_other: bool = False):
        ...
        self.fc = NightLinear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
...
class GatedConvBlock(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = NightLinear(dim, hidden * 2)
        self.act = act_layer()
        ...
        self.fc2 = NightLinear(hidden, dim)
...
class MambaOut(nn.Module):
    ...
    Args:
        ...
        output_norm: norm before classifier head. Default: partial(LayerNorm, eps=1e-6).
        head_fn: classification head. Default: NightLinear.
    ...
    def _init_weights(self, m):
        if isinstance(m, (nn.Conv2d, NightLinear)):
