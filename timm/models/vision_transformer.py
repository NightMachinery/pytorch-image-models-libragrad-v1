...
from timm.models.decomposition import FairAttentionPoolLatent as AttentionPoolLatent
...
class DeprecatedAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = NightLinear(dim, dim * 3, bias=qkv_bias)
        self.q_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = NightLinear(dim, dim)
...
class Block(nn.Module):
    def __init__(
            ...
            layerscale_layer=LayerScaleDecomposed,
            attention_layer=AttentionDecomposed,
    ) -> None:
        # assert init_values is None, "init_values enables LayerScale which is not currently decomposed"
        ...
        self.ls1 = layerscale_layer(dim, init_values=init_values) if init_values else nn.Identity()
        ...
        self.ls2 = layerscale_layer(dim, init_values=init_values) if init_values else nn.Identity()
        self.drop_path2 = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        # ic(self.ls1, self.ls2)
    ...
    def forward(self, inputs):
        ...
        ##
        #: doesn't help, but not a problem either:
        # torch_gpu_memory_stats()
        # torch_gpu_empty_cache()
        # # torch_gpu_remove_all()
        # torch_gpu_memory_stats()
        ...
        #: @perf/memory We cannot forcefully evict tensors in simple_obj_update; doing so will nuke our residuals. Our current GC-based approach works fine.

        inputs = self.norm1(inputs)
        inputs = self.attn(inputs)
        inputs = self.ls1(inputs)
        inputs = nondecomposed_forward(self.drop_path1, inputs)
        ...
        inputs = self.ls2(inputs)
        inputs = nondecomposed_forward(self.drop_path2, inputs)
        inputs = residual_decomposed(
            inputs=inputs,
            residuals=residuals,
            residual_attributions_v=residual_attributions_v,
...
class ParallelScalingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.in_proj = NightLinear(dim, in_proj_out_dim, bias=qkv_bias)
        ...
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.attn_out_proj = NightLinear(dim, dim)

        self.mlp_drop = nn.Dropout(proj_drop)
        self.mlp_act = act_layer()
        self.mlp_out_proj = NightLinear(mlp_hidden_dim, dim)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        act_layer = get_act_layer(act_layer) or GELU
    ...
    def pixels2patches(
        self,
        x,
        # decomposition_config: DecompositionConfig = None,
        # decompose_p=True,
    ):
        # print_diag(f"raw input: shape = {(x.shape)}", group="shape")

        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)

        # print_diag(f"input patched and pos-embedded: shape = {ic_colorize2(x.shape)}", group="shape")
        #: (224**2)/(16**2) = 196
        #: There is one CLS token, too, so 197

        return x


    def forward_features_patch_level_decomposed(self, *args, **kwargs):
        return self.forward_features_decomposed(*args, patchify_p=False, **kwargs)


    patch_level_forward_features_decomposed = forward_features_patch_level_decomposed #: @alias


    def forward_features_decomposed(
            self,
            x,
            decomposition_config: DecompositionConfig = None,
            decompose_p=True,
            patchify_p=True, #: whether to run pixels2patches on the input
    ):
        if decomposition_config is None:
            decomposition_config = DecompositionConfig(
                device=next(self.parameters()).device,
                pre_norm_include_p=self.pre_norm_p,)

        context_managers = dynamic_config_contexts(
        ...
        with ExitStack() as stack:
            ...
            if decompose_p:
                ...
                inputs = attribute_patches_aggregate(
                    inputs,
                    prefix="forward_features_decomposed.start.",
                    mode=decomposition_config.attributions_aggregation_strategy,
                    # mode='reset',
                    start_p=True,
                )
            ...
            inputs = self.norm_pre(inputs)

            if self.grad_checkpointing and not torch.jit.is_scripting():
                #: not tested with decompose_p
            ...
            check_attributions_v2(
                inputs,
                prefix=f"forward_features_decomposed.end.",
                assert_p=decomposition_config.assertion_check_attributions_end_p,
    ...
    def forward_features(self, *args, **kwargs):
        res = self.forward_features_decomposed(*args, decompose_p=False, **kwargs)
        return res
    ...
    def forward_head_decomposed(self, inputs, pre_logits: bool = False, decompose_p=True):
        ##
        #: When pre_logits==False, returns logits.
        #: So you need to use a softmax yourself.
        ##
        if decomposed_inputs_p(inputs):
            ...
            return self.forward_head(inputs, pre_logits=pre_logits)
    ...
    def forward_decomposed(self, x, return_features_p=False, **kwargs):
        inputs = self.forward_features_decomposed(x, **kwargs)
        ...
        inputs = self.forward_head_decomposed(inputs)

        print_diag(f"logits: attributions_v.shape: {(inputs.attributions_v.shape)}", group="shape")
    ...
    patch_level_forward_decomposed = forward_patch_level_decomposed  #: @alias

    def forward(self, x, patchify_p=True, keep_indices=None,):
        #: `keep_indices` is a dummy input here.
        ##
        context_managers = dynamic_config_contexts(
            decompose_p=False,
        )
        ...
        with ExitStack() as stack:
            ...
            x = self.forward_features(x, patchify_p=patchify_p)
...
def init_weights_vit_timm(module: nn.Module, name: str = '') -> None:
    ...
    if isinstance(module, NightLinear):
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
    ...
    if (isinstance(model.head, NightLinear) and
            f'{prefix}head/bias' in w and
            model.head.bias.shape[0] == w[f'{prefix}head/bias'].shape[-1]):
        model.head.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head.bias.copy_(_n2p(w[f'{prefix}head/bias']))
    # NOTE representation layer has been removed, not used in latest 21k/1k pretrained weights
    # if isinstance(getattr(model.pre_logits, 'fc', None), NightLinear) and f'{prefix}pre_logits/bias' in w:
...
def checkpoint_filter_fn(
    ...
    if 'visual.class_embedding' in state_dict:
        ...
        prefix = 'visual.trunk.'
        if 'visual.head.proj.weight' in state_dict and isinstance(model.head, NightLinear):
            # remap final NightLinear if it exists outside of the timm .trunk (ie in visual.head.proj)
...
def vit_xsmall_patch16_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    # TinyCLIP 8M
    model_args = dict(embed_dim=256, depth=10, num_heads=4, pre_norm=True, norm_layer=LayerNorm)
    model = _create_vision_transformer(
...
def vit_medium_patch32_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    # TinyCLIP 40M
    model_args = dict(
        patch_size=32, embed_dim=512, depth=12, num_heads=8, pre_norm=True, norm_layer=LayerNorm)
    model = _create_vision_transformer(
...
def vit_medium_patch16_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    # TinyCLIP 39M
    model_args = dict(embed_dim=512, depth=12, num_heads=8, pre_norm=True, norm_layer=LayerNorm)
    model = _create_vision_transformer(
...
def vit_betwixt_patch32_clip_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    # TinyCLIP 61M
    model_args = dict(
        patch_size=32, embed_dim=640, depth=12, num_heads=10, pre_norm=True, norm_layer=LayerNorm)
    model = _create_vision_transformer(
...
def vit_base_patch32_clip_quickgelu_224(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    model_args = dict(
        ...
        norm_layer=LayerNorm, act_layer='quick_gelu')
    model = _create_vision_transformer(
