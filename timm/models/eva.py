...
class EvaAttention(nn.Module):
    ...
    def __init__(
        ...
        if qkv_fused:
            ...
            self.qkv = LinearDecomposed(dim, all_head_dim * 3, bias=qkv_bias)
            ##
            # self.qkv = LinearDecomposed(dim, all_head_dim * 3, bias=False)
            ...
            self.q_proj = LinearDecomposed(dim, all_head_dim, bias=qkv_bias)
            self.k_proj = LinearDecomposed(dim, all_head_dim, bias=False)
            self.v_proj = LinearDecomposed(dim, all_head_dim, bias=qkv_bias)
            self.qkv = None
            self.q_bias = self.k_bias = self.v_bias = None

        self.attn_drop = nn.Dropout(attn_drop)
        self.norm = norm_layer(all_head_dim) if norm_layer is not None else nn.Identity()
        self.proj = LinearDecomposed(all_head_dim, dim)
    ...
    def forward(
        ...
        if self.qkv is not None:
            ##
            assert not self.qkv_bias_separate, "My fork does not support this, as we need the biases in the Linear module for FullGrad."
            qkv = self.qkv(x)
            ##
            #: Our fork changes the weight loading such that the biases get loaded into the Linear module.
...
class Eva(nn.Module):
    ...
    def __init__(
        ...
        self.head = LinearDecomposed(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        ...
        if isinstance(self.head, LinearDecomposed):
    ...
    def _init_weights(self, m):
        if isinstance(m, LinearDecomposed):
    ...
    def reset_classifier(self, num_classes: int, global_pool: Optional[str] = None):
        ...
        if global_pool is not None:
            self.global_pool = global_pool
        self.head = LinearDecomposed(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def _pos_embed(self, x, keep_indices=None,) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        if self.dynamic_img_size:
            raise NotImplementedError("Fair EVA with dynamic_img_size has not been implemented.")
        ...
        x = self.pos_drop(x)
        #: We don't want to put the dropout into `pixels2patches`.

        # obtain shared rotary position embedding and apply patch dropout
        if rot_pos_embed is not None and keep_indices is not None:
            #: make sure model is NOT in training, as we cannot apply both patch dropping via faithfulness and patch_drop.
            assert not self.training

            # ic(x.shape, keep_indices.shape, rot_pos_embed.shape)
            # ic| x.shape: torch.Size([36, 58, 384])
            # keep_indices.shape: torch.Size([36, 58])
            #: rot_pos_embed.shape: torch.Size([576, 128])

            keep_indices = keep_indices[:, self.num_prefix_tokens :] - self.num_prefix_tokens
            #: Rope is only used in the attention module, where CLS is excluded (hardcoded, too, so we might also want to replace `num_prefix_tokens` with 1).
            # ic(keep_indices.shape)
            #: ic| keep_indices.shape: torch.Size([36, 57])

            rot_pos_embed = apply_keep_indices_nlc(x, rot_pos_embed, keep_indices)
            # ic(torch_shape_get(rot_pos_embed))
            # ic| torch_shape_get(rot_pos_embed): (torch.float32, torch.Size([36, 58, 128])

        elif self.patch_drop is not None:
    ...
    def forward_features(
        ...
        x, rot_pos_embed = self._pos_embed(x, keep_indices=keep_indices,)
