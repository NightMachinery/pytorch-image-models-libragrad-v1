...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        #: We import these here to avoid a circular import.
        ...
        conv2d_layer = NightConv2d
        # conv2d_layer = nn.Conv2d
        self.proj = conv2d_layer(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, bias=bias)

        if norm_layer:
            if isinstance(norm_layer, nn.LayerNorm):
                print_diag("PatchEmbed uses LayerNorm", group="patch_embed_verbose")

                self.norm = LayerNormDecomposed(embed_dim)
            else:
                #: Let's just raise an error if we got another layer ...
