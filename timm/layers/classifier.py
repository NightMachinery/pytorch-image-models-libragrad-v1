...
from timm.night.decomposition import LinearDecomposed as NightLinear
...
def _create_fc(num_features, num_classes, use_conv=False):
    if num_classes <= 0:
        ...
        fc = NightLinear(num_features, num_classes, bias=True)
    return fc
...
class NormMlpClassifierHead(nn.Module):
    ...
    def __init__(
        ...
        linear_layer = partial(nn.Conv2d, kernel_size=1) if self.use_conv else NightLinear
    ...
    def reset(self, num_classes: int, pool_type: Optional[str] = None):
        ...
        if self.hidden_size:
            if ((isinstance(self.pre_logits.fc, nn.Conv2d) and not self.use_conv) or
                    (isinstance(self.pre_logits.fc, NightLinear) and self.use_conv)):
                with torch.no_grad():
